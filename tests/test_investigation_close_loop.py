...
class FailingRunManager:
    ...
    def create_evaluation(self, request: EvaluationRequest) -> SimpleNamespace:
        self.counter += 1
        evaluation_id: str = f"eval_{self.counter}"
        evaluation: SimpleNamespace = SimpleNamespace(evaluation_id=evaluation_id, request=request)
...
def test_failed_investigation_waits_for_approval_then_regresses() -> None:
    ...
    manager: FailingRunManager = FailingRunManager()
    ...
    pending: dict[str, object] = session.snapshot()
